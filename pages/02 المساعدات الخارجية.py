...
df_long = df.melt(id_vars="Sector", var_name="Year", value_name="Spending")
df_long["Year"] = df_long["Year"].str.replace("Spending in ", "")
...
with col1:
    selected_year = st.radio(
        ...
        options=["2022", "2023", "2024"],
        index=2,
        key="sector_year_selector"
    )

# تصفية البيانات على السنة المختارة
df_selected = df_long[df_long["Year"] == selected_year]
...
fig = px.bar(
    ...
    color="Sector",
    text="Spending",
    title=f"📊 الإنفاق عبر القطاع - {selected_year}",
    height=600
)
...
fig.update_layout(
    xaxis_title="Sector",
    yaxis_title="Spending (Million AED)",
    showlegend=False,  # نخفي الليجند لأن كل عمود يمثل القطاع نفسه
    width=800,
    title_x=0.5
)
